 ...
 */
const getNodeIntersection = (intersectionNode: InternalNode, targetNode: InternalNode, intersectionField: string): XYPosition => {
  ...
  const padding = 8;
  const fieldIndex = intersectionNode.data.fields.findIndex(field => field.name === intersectionField);
  console.log('intersectionNode', intersectionNode.data.fields, intersectionField, fieldIndex);
  const targetPosition = targetNode.position;

  // const w = (intersectionNodeWidth ?? 0) / 2;
  // const h = (intersectionNodeHeight ?? 0) / 2;
  const w = intersectionNode.position.x < targetNode.position.x
    ? intersectionNodeWidth
    : 0;
  const fieldHeight = (intersectionNodeHeight - headerHeight - 2 * padding) / intersectionNode.data.fields.length;
  const h = intersectionNodeHeight
    ? headerHeight + padding + (fieldHeight * fieldIndex) + (fieldHeight / 2)
    : 0;
  console.log({ intersectionNodeHeight, fields: intersectionNode.data.fields.length, fieldHeight, h });
 ...
 */
export const getEdgeParams = (source: InternalNode, target: InternalNode, sourceField: string, targetField: string) => {
  const sourceIntersectionPoint = getNodeIntersection(source, target, sourceField);
  const targetIntersectionPoint = getNodeIntersection(target, source, targetField);
